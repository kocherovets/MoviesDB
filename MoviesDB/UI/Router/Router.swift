...
class Router {
    ...
    static func showMovie(movieId: Int, title: String) {

        ui {
            let vc = createVC(storyboardName: "Movie", type: MovieVC.self)
            vc.movieTitle = title
            if let presenter = vc.presenter as? MovieVCModule.Presenter {
